...
if sqrt(x^2+y^2+z^2) <= 0.5 % still on rails
    ...
    % conversion measures from the positive-z (straight down)
...
% Sum of the forces
Fx = (hBod_x * -(D+f_rails)) + (hBod_x * T);
Fy = (hBod_y * -(D+f_rails)) + (hBod_y * T);
Fz = (hBod_z * -(D+f_rails)) + (hBod_z * T) + (g*m);
